    ...
    <div className="job-list-card card">
      ...
      <div className="jlc-middle">
        ...
        <div className="jlc-meta">
          ...
          <span><Briefcase size={12} /> {job.experienceMin ?? 0}–{job.experienceMax ?? 2} yrs</span>
      ...
      <div className="jlc-right">
        <div className="jlc-salary">{formatSalary(job.salaryMin, job.salaryMax)}</div>
        <a href={job.applyLink || '#'} target="_blank" rel="noopener noreferrer" className="btn btn-primary btn-sm">Apply Now</a>
...
export default function PrivateJobs() {
  ...
  const fetchJobs = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await jobService.search({ type: activeTab, search: search || undefined, page, size: 20 });
    ...
    <div className="private-jobs-page">
      <div className="page-header-banner">
        <div className="container">
          ...
          <h1 className="page-title">{activeTab === 'INTERNSHIP' ? 'Latest Internships 2024' : 'Private Sector Jobs 2024'}</h1>
          <p className="page-subtitle">Find jobs at top companies matching your skills and goals</p>
          <div className="page-stats">
            <span>💼 15K+ Jobs</span><span>🏢 500+ Companies</span><span>✅ Verified Listings</span>
          </div>
        </div>
      </div>
      ...
      <div className="pj-tabs-bar">
        <div className="container" style={{ display: 'flex', gap: 0 }}>
          <button className={`pj-tab ${activeTab === 'PRIVATE' ? 'active' : ''}`} onClick={() => { setActiveTab('PRIVATE'); setPage(0); }}>💼 Full-time Jobs</button>
          <button className={`pj-tab ${activeTab === 'INTERNSHIP' ? 'active' : ''}`} onClick={() => { setActiveTab('INTERNSHIP'); setPage(0); }}>🎓 Internships</button>
        </div>
      </div>
          ...
          <div>
            <div className="pj-filter-bar card-flat">
              <div className="search-input-wrap" style={{ flex: 2 }}>
                ...
                <input className="search-bar-input" placeholder="Search job title, company, skill..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(0); }} />
              </div>
              <button className="btn btn-primary" onClick={fetchJobs}><SlidersHorizontal size={14} /> Search</button>
            </div>

            {error && <div className="error-state"><p>⚠️ {error}</p><button className="btn btn-outline btn-sm" onClick={fetchJobs}>Retry</button></div>}

            {loading ? (
              <div className="loading-state"><div className="spinner" /><p>Loading jobs...</p></div>
            ) : (
              <>
                <div className="results-count" style={{ marginTop: 12 }}>Showing <strong>{jobs.length}</strong> {activeTab === 'INTERNSHIP' ? 'internships' : 'jobs'}</div>
                <div className="job-list" style={{ marginTop: 12 }}>
                  {jobs.map((job) => <JobListCard key={job.id} job={job} />)}
                  {jobs.length === 0 && <div className="empty-state"><div className="empty-icon">💼</div><h3>No jobs found</h3><p>Try different search or filter</p></div>}
                </div>
                {totalPages > 1 && (
          ...
          <div>
            <div className="sidebar-widget card-flat">
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {['React.js','Python','Java','SQL','Node.js','Machine Learning','AWS','Docker'].map((s) => <span key={s} className="chip">{s}</span>)}
              </div>
            </div>
            <div className="sidebar-widget card-flat" style={{ marginTop: 16 }}>
              <h3 className="sw-title">🤖 AI Career Matcher</h3>
              <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 12 }}>Let AI find jobs perfectly matching your profile</p>
              <Link to="/ai-advisor" className="btn btn-primary w-full" style={{ justifyContent: 'center' }}>Find My Jobs →</Link>
            </div>
          </div>
        </div>
      </div>
