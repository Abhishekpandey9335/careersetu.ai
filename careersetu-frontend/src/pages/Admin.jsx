    ...
    <div style={{ minHeight: '100vh', background: '#f8fafc', padding: '24px' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        {stats && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 14, marginBottom: 24 }}>
            {[
              ...
              { label: 'Pending Payment', value: stats.pendingSubscriptions, icon: '⏳', highlight: true },
            ].map(s => (
              <div key={s.label} style={{
                ...
                <div style={{ fontSize: 26, fontWeight: 800, color: s.highlight ? '#d97706' : '#1e293b' }}>{s.value}</div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
          <button onClick={() => switchTab('payments')} style={{
            padding: '10px 20px', borderRadius: 8, border: 'none', cursor: 'pointer',
            fontWeight: 700, fontSize: 14,
            background: activeTab === 'payments' ? '#6366f1' : '#e2e8f0',
            color: activeTab === 'payments' ? '#fff' : '#475569'
          }}>
            💳 Payments
          </button>
          <button onClick={() => switchTab('users')} style={{
            padding: '10px 20px', borderRadius: 8, border: 'none', cursor: 'pointer',
            fontWeight: 700, fontSize: 14,
            background: activeTab === 'users' ? '#6366f1' : '#e2e8f0',
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
              {['PENDING', 'ACTIVE', 'FAILED', 'ALL'].map(f => (
                <button key={f} onClick={() => changeFilter(f)} style={{
                  ...
                  background: filter === f ? '#0e9f6e' : '#e2e8f0',
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                      <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: 13, fontWeight: 700, color: '#475569' }}>#</th>
                      <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: 13, fontWeight: 700, color: '#475569' }}>Name</th>
                      <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: 13, fontWeight: 700, color: '#475569' }}>Email</th>
                      <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: 13, fontWeight: 700, color: '#475569' }}>Role</th>
                      <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: 13, fontWeight: 700, color: '#475569' }}>Joined</th>
                    </tr>
                  </thead>
                  <tbody>
                    {users.map((u, i) => (
